...
import { actDeleteUser, actListAdminUser } from './duck/action';
...
export default function UserAdminPage() {
    ...
    const loading = useSelector(state => state.adminUserReducer.loading);
    const data = useSelector(state => state.adminUserReducer.data);
    const dispatch = useDispatch()

    useEffect(() => {
        dispatch(actListAdminUser())
    }, []);
            ...
            return data.map((item) => {
                return (
                    <tr key={item.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td className="px-6 py-4">
                            <button type="button" className="font-medium text-blue-600 dark:text-blue-500 hover:underline" onClick={()=>{handleEdit(item.id)}} data-modal-target="modalEl" data-modal-toggle="modalEl" >Edit</button>
                            <button type='button' onClick={()=>{
                                if(window.confirm('bạn có chắc xóa')){
                                    dispatch(actDeleteUser(item.id));
                                    window.location.reload(false);
                                };
                            }} className="font-medium text-blue-600 dark:text-blue-500 hover:underline ml-3" >Delete</button>
                        </td>
                    </tr>
                )
            })
    ...
    if(loading) return  <div className='p-4 sm:ml-64 mt-16'><Loader /></div> 
    return (
        ...
        <>
            <div className="p-4 sm:ml-64 mt-16">
                ...
                <div className="relative overflow-x-auto shadow-md sm:rounded-lg ">
                    <div className="flex items-center justify-between pb-4">
                        <div>
                            <button onClick={()=>{ setEdit(0)}} data-modal-target="modalEl" data-modal-toggle="modalEl"  className="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                                <svg className="w-5 h-5 fill-white dark:text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">{/*! Font Awesome Pro 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. */}<path d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z" /></svg>
            ...
            {/* Modal */}
            <div id="modalEl" tabIndex={-1}  aria-hidden="true" className="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
                <div className="relative w-full h-full max-w-md md:h-auto">
                    {/* Modal content */}
                    <div className="relative bg-gray-200 rounded-lg shadow dark:bg-gray-700">
                        <button type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="modalEl">
                            <svg aria-hidden="true" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                            <span className="sr-only">Close modal</span>
                        </button>

                         {isEdit === 0 ? <AddUser />: <UpdateUser id={isEdit}  />}
                    </div>
                </div>
            </div>
        </>
